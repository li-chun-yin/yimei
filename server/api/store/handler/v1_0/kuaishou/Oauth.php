...
use api\store\parameter\v1_0\kuaishou\oauth\OauthResponse;
 ...
 * @request api\store\parameter\v1_0\kuaishou\oauth\OauthRequest
 * @response api\store\parameter\v1_0\kuaishou\oauth\OauthResponse
 *
...
class Oauth implements ApiClassInterface
{
    ...
    {
        try {
            $DouyinSettingEntity  = $this->SettingManager->loadByType(Code::TYPE_KUAISHOU);

            if(empty($DouyinSettingEntity->getData()['app_id'])){
                throw new SystemException('系统尚未配置抖音open api clent info');
            }

            return new OauthResponse(
                $this->Router,
                $DouyinSettingEntity->getData()['app_id'],
